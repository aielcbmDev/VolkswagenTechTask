...
class WeatherDatabaseDataSourceIOSTest {

    @Test
    fun verify_datastore_data_is_saved_and_retrieved_successfully() = runTest {
        // GIVEN
        val weatherDatabase =
            Room.inMemoryDatabaseBuilder<WeatherDatabase>().setDriver(BundledSQLiteDriver()).build()
        val dailyForecastDao = weatherDatabase.getDailyDao()
